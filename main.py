...
def main():
    ...
    # BTC
    # =========================
    btc_compressed = build_compressed_vector(originals, aplicar_compressao_btc)

    btc_seg_orig = build_segmented_vector(originals, segmenter.closing_pipeline)

    btc_seg_comp = build_segmented_vector(btc_compressed, segmenter.closing_pipeline)

    evaluate("CLOSING + BTC", btc_seg_orig, btc_seg_comp)

    btc_seg_orig_w = build_segmented_vector(
        originals, segmenter.sobel_watershed_pipeline
    )

    btc_seg_comp_w = build_segmented_vector(
        btc_compressed, segmenter.sobel_watershed_pipeline
    )

    evaluate("WATERSHED + BTC", btc_seg_orig_w, btc_seg_comp_w)

    # =========================
    # DCT
    # =========================
    dct_compressed = build_compressed_vector(originals, frequency_compression_step)

    dct_seg_orig = build_segmented_vector(originals, segmenter.closing_pipeline)

    dct_seg_comp = build_segmented_vector(dct_compressed, segmenter.closing_pipeline)

    evaluate("CLOSING + DCT", dct_seg_orig, dct_seg_comp)
    ...
    dct_seg_comp_w = build_segmented_vector(
        dct_compressed, segmenter.sobel_watershed_pipeline
    )

    evaluate("WATERSHED + DCT", dct_seg_orig_w, dct_seg_comp_w)
